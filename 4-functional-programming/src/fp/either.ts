 ...
 */
export const map = <E, A, B>(fn: (a: A) => B) => (fa: Either<E, A>): Either<E, B> => ( isRight(fa)?{_tag:'Right', right:fn(fa.right)} : {_tag:'Left', left:fa.left});
 ...
 */
export const fromPromise = <E, A>(promise: Promise<A>): Promise<Either<E, A>> => (promise.then((val)=>right(val)).catch((err:E)=>left(err)));

//export const fromPromise = <A>(promise: Promise<A>): Promise<Either<A, A>> => ( promise.then((val)=>Promise.resolve(right(val))).catch((val)=>left(val)) );
/**
 ...
 * reduce Array<A> => B
 */
export const fold = <E, A, B>(onLeft: (e: E) => B, onRight: (a: A) => B) => (ma: Either<E, A>): B => ( isLeft(ma)? onLeft(ma.left) : onRight(ma.right));
